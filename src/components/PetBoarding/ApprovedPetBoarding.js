...
import { PetBoardingAndStoreColumn, APPROVED_PET_BOARDING } from '../../constants'
import { connect } from 'react-redux';

function ApprovedPetBoarding (props) {
    ...
    const onDataChangedPetBoardings = ( items ) => {
        let profile = [];
    ...
    useEffect(() => {
        dataservice.getAll( APPROVED_PET_BOARDING ).on("value", onDataChangedPetBoardings);
    
        return () => {
            dataservice.getAll( APPROVED_PET_BOARDING ).off("value", onDataChangedPetBoardings);
        };
      }, []);
    ...
    const rowEvents = {
        onDoubleClick: (e, row) => {
           ...
           history.push('/SingleApprovedBoarding');
        }
    }
    const selectRow = {
        mode: "radio",
        ...
        <div>
            <div className = "row justify-content-md-center mb-3">
                <div className="col-lg-auto">
                    <h2 className = "">Pet Boarding List</h2>
                </div>
            </div>
...
export default connect (null, mapDispatchToProps ) (ApprovedPetBoarding)
