...
import { PetBoardingAndStoreColumn, PENDING_PET_STORE } from '../../constants'

function PendingPetStore (props) {
    ...
    useEffect(() => {
        dataservice.getAll( PENDING_PET_STORE ).on("value", onDataChangedPetStores);
    
        return () => {
            dataservice.getAll( PENDING_PET_STORE ).off("value", onDataChangedPetStores);
        };
      }, []);
    ...
    const rowEvents = {
        onDoubleClick: (e, row) => {
           ...
           history.push('/SinglePendingPetStore');
        }
    }
    const selectRow = {
        mode: "radio",
        ...
        <div>
            <div className = "row justify-content-md-center mb-3">
                <div className="col-lg-auto">
                    <h2 className = "">Pet Store Pending List</h2>
                </div>
            </div>
...
export default connect (null, mapDispatchToProps) (PendingPetStore)
